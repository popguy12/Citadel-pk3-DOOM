class Citadel_PlayerHandler : EventHandler
{
	int kicktimer;
	...
	override void Tick()
	{
		...
		if (f_enabled && (playerVel2D > 0.1) && (toFollow.pos.z - toFollow.floorz <= 0)) {			
			...
			if (floorpic == skyflatnum)
				stepsound = "none";
			else if (toFollow.CountInv("IsProne") || toFollow.CountInv("Kicking") || toFollow.CountInv("Sliding"))
			...
			if (stepsound != "none" || !toFollow.CountInv("Sliding"))
			{
				if (stepsound == "step/dirt")
				{
					toFollow.A_StartSound("step/defaultA", CHAN_AUTO, CHANF_LOCAL|CHANF_UI|CHANF_OVERLAP, volume:0.25);
				}
				toFollow.A_StartSound("step/highheel", CHAN_AUTO, CHANF_LOCAL|CHANF_UI|CHANF_OVERLAP, volume:0.8);
				toFollow.A_StartSound(stepsound, CHAN_AUTO, CHANF_LOCAL|CHANF_UI, volume:soundVolume * 0.5);
				...
				*/
				...
				toFollow.A_StartSound("metal/accent", CHAN_AUTO, CHANF_LOCAL|CHANF_UI|CHANF_OVERLAP, volume: (toFollow.CountInv("BasicArmor") / 200));
