...
{
	action void Citadel_WeaponReady(int weapflags = 0)
	{
	...
	/*
	action void Citadel_LowAmmoSoundWarning(string type = "default", string ammoItem = "SuperMario64ForTheXboxOne")
	{
		Ammo count;
		...
		if(!count)
		{
			console.printf("Citadel_LowAmmoSoundWarning: Invalid ammo class");
			return;
		}
		...
		{
			double clickvolume = Citadel_Math.LinearMap(currentammocount, 1.0, lowammocount, 1.0, 0.5);
			...
			if(currentammocount == 0)
				console.printf("Citadel_LowAmmoSoundWarning: Ammo count is 0 instead of 1, was the function called after the ammo item was taken?");
		}
	}
	*/
	//- L, 0 C, + R
	// [gng] credits to jaih1r0 for the position calculation, modified by me to fix the pitch bug
	Action void Citadel_GunSmoke(double d1 = 0, double d2 = 0 , double d3 = 0, string SActor = "Citadel_GunFireSmoke")
	{
		double PVZ;
		CitadelPlayer plr = CitadelPlayer(invoker.owner);
		...
		Citadel_GunFireSmoke Smoke = Citadel_GunFireSmoke(Spawn(SActor, spos));
		
		If(Smoke)
		{
			Smoke.master = invoker.owner;
			//Smoke.Vel = Citadel_Math.AngleToVector3D(self.Angle, -self.Pitch);
	...
	action void Citadel_RaiseWeapon() {
		A_Raise(25);
	}
	
	action void Citadel_LowerWeapon() {
		A_Lower(25);
	}
	/*
	...
	{
        return (owner.CountInv("Citadel_PowerStrength") >= 1);
	}
	
	//Adds weapon recoil, modifying it if the owner has a berserk powerup
	action void Citadel_WeaponRecoil(float pitchDelta, float angleDelta, float powerMod = 0.5)
	{
        if (invoker.OwnerHasBerserk())
            Citadel_WeaponRecoilBasic(pitchDelta * powerMod, angleDelta * powerMod);
        else
            Citadel_WeaponRecoilBasic(pitchDelta, angleDelta);
	}
	*/
	//Normally you want to use PB_WeaponRecoil instead, because it accounts for the berserk powerup
	action void Citadel_WeaponRecoilBasic(float pitchDelta, float angleDelta = 0)
	{
		double fac = 1.0;
	...
	Action State Citadel_SpawnCasing(String Missile,Double ShellX,Double ShellY,Double ShellZ,Double ShellV_X,Double ShellV_Y,Double ShellV_Z)
	{
	...
	action void Citadel_AmmoIntoMag(String AmmoMag_Action,String AmmoPool_Action,int MagazineMaxFill_Action, int takeReserve)
		{
	...
	action void Citadel_UnloadMag(String AmmoMag_Action,String AmmoPool_Action, int giveReserve)
	{
	...
	action void Citadel_FireBullets(string type,int amount,float angle,double offs,double height,float pitch)
	{
	...
	action void Citadel_QuakeCamera(int qDur, float camRoll)
	{
	...
	action void Citadel_HandleWeaponFeedback(int qDur, float camRoll, float pitchDelta, float angleDelta, double d1 = 0, double d2 = 0 , double d3 = 0)
	{
		Citadel_QuakeCamera(qDur, camRoll);
		Citadel_WeaponRecoilBasic(pitchDelta, angleDelta);
		Citadel_GunSmoke(d1, d2, d3);
	}
	
	action void Citadel_HandleCrosshair(int num)
	{
        CVar crosshair_settings = CVar.FindCVar('Citadel_weapon_crosshairs');
