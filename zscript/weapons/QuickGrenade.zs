	...
	States
	{
		...
		Explode:
			TNT1 A 0
			{
				...
				Spawn('Citadel_Explosion', pos);
				A_AlertMonsters(4096);
				A_StartSound("effect/explosion", 0, CHANF_OVERLAP);
				for(int i=4;i>0;i--)
				{
					Spawn("Citadel_ExplosionSmoke", Level.Vec3Offset(pos, (frandom(10,-10),frandom(10,-10),frandom(10,-10))));
				}
			}
			Stop;
	}
		...
		Use:
			TNT1 A 0 
			{
				if (invoker.CooldownTimer >= 20)
				{
					...
					invoker.Owner.A_SelectWeapon("Citadel_Holster");
				}
			}
			TNT1 A 20;
			fail;
