...
class SwayManager : Thinker
{
	...
	// Check if weapon is in ADS mode
	bool IsWeaponInADS(PlayerInfo player)
	{
		if (!player || !player.ReadyWeapon)
			return false;
		
		// Check if it's a CustomReloadPistol
		let customPistol = CustomReloadPistol(player.ReadyWeapon);
		if (customPistol && customPistol.bInADS)
			return true;
		
		// Add checks for other weapons with ADS here in the future
		// Example: let otherWeapon = OtherWeaponClass(player.ReadyWeapon);
		// if (otherWeapon && otherWeapon.bInADS) return true;
	...
	override void Tick()
	{
		for (uint i = 0; i < MAXPLAYERS; ++i)
		{
			...
			// ADDED: Skip sway if weapon is in ADS
			if (IsWeaponInADS(players[i]))
			{
				// Reset offsets when entering ADS
				xOffset[i] = 0;
				yOffset[i] = 0;
				vOffset[i] = 0;
			...
			{
				if (pspr.id == PSP_WEAPON || pspr.id == 5)
					continue;
